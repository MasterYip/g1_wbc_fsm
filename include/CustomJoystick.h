...
// Forward declare to avoid REMOTE_DATA_RX conflict with gamepad.hpp
namespace unitree { namespace common { class UnitreeJoystick; } }
...
public:
    ...
    CustomJoystick(const std::string& device = "/dev/input/js0")
    {
        fd_ = open(device.c_str(), O_RDONLY | O_NONBLOCK);
        if (fd_ < 0) {
            std::cerr << "CustomJoystick: failed to open '" << device
                      << "' (" << strerror(errno) << ")" << std::endl;
        } else {
            std::cout << "CustomJoystick: reading from '" << device << "'" << std::endl;
        }
    }
    ...
    bool is_connected() const { return fd_ >= 0; }
        ...
        // ---- public accessors for direct bridge ----
    float axis(int idx) const {
        if (idx < 0 || idx > 5) return 0.0f;  //0->lx, 1->ly, 3->rx, 2->ry, 4->RT, 5->LT
        static const int logic_to_phys[6] = {0,1,2,3,4,5};
        return axes_.at(logic_to_phys[idx]);
    }

    int button(int idx) const {
        if (idx < 0 || idx > 7) return 0;  //0->A, 1->B, 3->X, 4->Y, 6->LB, 7->RB， 10->back, 11->start
        static const int logic_to_phys[8] = {0,1,3,4,6,7,10,11};
        return buttons_.at(logic_to_phys[idx]);
    }
    int dpad_x() const { return dpad_x_; }
    int dpad_y() const { return dpad_y_; }
    ...
    void poll()
    {
        ...
        while ((n = read(fd_, &ev, sizeof(ev))) == sizeof(ev)) {
            ...
            case JS_EVENT_AXIS:
                if (ev.number < static_cast<int>(axis_raw_.size())) {
                    ...
                    // Normalise joystick axes (0,1,2,3) to [-1, 1]
                    if (ev.number == 0 || ev.number == 1 ||
                        ev.number == 2 || ev.number == 3) {
                        axes_[ev.number] = std::clamp(
                            ev.value / 32767.0f, -1.0f, 1.0f);
                    }
                    // Auto-calibrate trigger axes (4=l2, 5=r2)
                    if (ev.number == 4) {
        ...
        axes_[4] = norm_trigger(axis_raw_[4], l2_min_, l2_max_);
...
private:
    ...
    std::array<float, 10> axes_{};

    /// Raw axis values for trigger auto-calibration
    std::array<float, 10> axis_raw_{};
    ...
    /// Auto-calibration range for l2 (axis 4)
